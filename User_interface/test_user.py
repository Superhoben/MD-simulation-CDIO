...
def initiate_gui():
    ...
    sep_label_1 = Label(data_frame, text="-"*90, bg = "medium aquamarine")
    sep_label_1.grid(row=14, column = 0, columnspan=3)
    ...
    sep_label_2 = Label(data_frame, text="-"*90, bg = "medium aquamarine")
    sep_label_2.grid(row=17, column = 0, columnspan=3)
    ...
    sep_label_3 = Label(data_frame, text="-"*90, bg = "medium aquamarine")
    sep_label_3.grid(row=22, column = 0, columnspan=3)
    ...
    plot_button = Button(data_frame, text='Plot data',
                            command=lambda: visualise_2D(value_inside_plottable_list.get(), value_inside_output_data.get(), ax_canvas))
...
def visualise_2D(attribute_to_plot, file_to_plot, ax_canvas):
    ...
    while i < len(material_data_dict[attribute_to_plot.lower()]):
        x_axis.append(i * int(config_data['SimulationSettings']['time_step']) * 
    ...
    plot(ax_canvas[0], ax_canvas[1], x_axis, material_data_dict[attribute_to_plot.lower()], x_lim)
